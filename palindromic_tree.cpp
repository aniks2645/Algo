...
void pinitialize()
{
    ...
    pt[0].link = 0, pt[0].len = -1;
    pt[1].link = 0, pt[1].len = 0;
    sz = cur = 1;
}
void extend(int p)
{
    while(s[p-pt[cur].len-1] != s[p]) cur = pt[cur].link;
    int x = pt[cur].link;
    while(s[p-pt[x].len-1] != s[p]) x = pt[x].link;

    int c = s[p] - 'a';
    if(!pt[cur].next[c])
    {
        pt[cur].next[c] = ++sz;
        pt[sz].len = pt[cur].len + 2;
        if(pt[sz].len==1) pt[sz].link = 1;
        else pt[sz].link = pt[x].next[c];
    }
    cur = pt[cur].next[c];
}
int main() {
