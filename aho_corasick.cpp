...
//*/
const int M = 3*MM;
int node[M][26], fail[M], epos[M], res[M], path[M], psz, nnode;

void initialize(){
    SET(node); mem(fail); mem(path); mem(res);
    nnode = psz = 0;
}

void addAC(string s, int id){
    int now = 0;
    for(auto x : s){
        char ch = x - 'a';
        if(node[now][ch] == -1){
...
void buildAC(){
    ...
    for(int i=0; i<26; i++){
        ...
        else              node[0][i] = 0; // because of automation
    }
    while(q.size()){
        int x = q.front(); q.pop();
        for(int i=0; i<26; i++){
            ...
            if( now != -1){
                q.push(now);
                fail[now] = node[fail[x]][i]; // no need kmp
                path[++psz] = now;
            }
            else {
                node[x][i] = node[fail[x]][i]; // automates and eliminate kmp to find failure link
            }
        }
    }
}

void queryAC(string s){
    int now = 0;
    for(auto x : s){
        now = node[now][x - 'a'];
        res[now]++;
