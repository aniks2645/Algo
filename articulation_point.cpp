...
//*/
vi adj[MM];
ll low[MM], dis[MM],cnt, root,vis[MM], cn,res,point[MM];

void articulation( int x, int p )
{
    dis[x] = ++cnt;
    low[x] = cnt;
    vis[x] = 1;
    ...
    rep(sz)
    {
        ...
        {
            if(x==root)cn++;
            articulation(adj[x][i],x);
            low[x] = min(low[x],low[adj[x][i]]);
            if(dis[x]<=low[adj[x][i]] && x!=root)
            {
                point[x] = 1;
            }
        }
        else low[x] = min(low[x],dis[adj[x][i]]);
    }
    if(x==root && cn>1)point[x] = 1;
}
int main() {
